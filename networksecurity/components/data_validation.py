...
class DataValidation:
    ...
    def validate_is_numerical(self, dataframe: pd.DataFrame)->bool:
        try:
    ...
    def detect_dataset_drift(self, base_df, current_df, threshold=0.05)->bool:
        try:
            ...
            for column in base_df.columns:
                ...
                dist_true=ks_2samp(d1,d2)
                if threshold <=dist_true.pvalue:
                    is_found=False
                else:
                    is_found=True
                    status=False
                report.update({column:{
                    "p_value": float(dist_true.pvalue),
    ...
    def initiate_data_validation(self)->DataValidationConfig:
        try:
            ...
            status=self.validate_is_numerical(dataframe=train_dataframe)
            if not status:
                error_message=f"{error_message} Column in Train dataframe is not numeric. \n"
            status=self.validate_is_numerical(dataframe=test_dataframe)
            if not status:
                error_message=f"{error_message} Column in Test dataframe is not numeric. \n"
